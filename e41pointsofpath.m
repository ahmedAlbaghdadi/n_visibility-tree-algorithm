...
for i=1 :myp(2)-1
 ...
 if mod(i , 2) ==0
     ...
     myconnect = connecttrobotpath(sphers(points(P(i) , 4) , 1) , sphers(points(P(i) , 4) , 2) , sphers(points(P(i) , 4) , 3) , sphers(points(P(i) , 4) , 4) , points(P(i) , 1) ,  points(P(i) , 2) ,  points(P(i) , 3) , points(P(i+1) , 1) ,  points(P(i+1) , 2) ,  points(P(i+1) , 3),1);
 b=int32(192);
nn=idivide(size(myconnect,2),b);
     newmyconnect=ones(3,nn);
c=int32(0);
 for dl=1:16:size(myconnect,2)
     c=idivide(dl,b,'ceil');
     newmyconnect(:,c) = myconnect(:,dl);
 end
     pointsofpath = [pointsofpath , newmyconnect];
 else
      strightpath = moverobotpath(points(P(i) , 1) ,  points(P(i) , 2) ,  points(P(i) , 3) , points(P(i+1) , 1) ,  points(P(i+1) , 2) ,  points(P(i+1) , 3) , 1);
  pointsofpath = [pointsofpath , strightpath];

 end
end
numberofpoints=length(pointsofpath(1,:));
simulinktime=100;
xd=timeseries([pointsofpath(1,:)]+10,simulinktime/numberofpoints:simulinktime/numberofpoints:simulinktime);
yd=timeseries([pointsofpath(2,:)]+11,simulinktime/numberofpoints:simulinktime/numberofpoints:simulinktime);
zd=timeseries([pointsofpath(3,:)]+10,simulinktime/numberofpoints:simulinktime/numberofpoints:simulinktime);
