function sure = connecttrobotpath(cx, cy ,cz, r, px, py, pz, qx , qy , qz , rr)
...
global sphers;
global lengthofpath;
niphor = dis( px, py, pz, qx , qy , qz)/2 ;
theta= (22/7)-2*acos(niphor/r);
lengthofpath = lengthofpath + theta*r;
...
stepy=-1;
 ...
 if abs(col1-col2)<=flength/2
     ...
     xsize=size(xa);
     xstep = floor(xsize(2)/10);
             plotsphersonly (sphers);
         hold on;
     for ste=1:xstep:xsize(2)
% 
%          q=0 : pi/50 : 2*pi;
%          w = 0 : pi/50 : 2*pi;
%          [q,w]=meshgrid(q,w);
%          xr= xa(ste)+rr*sin(q).*cos(w);
%          yr= ya(ste)+rr*sin(q).*sin(w);
%          zr = za(ste)+rr*cos (q);
         DrawQuad( xa(ste), ya(ste), za(ste), rr , 'r' , 'b')
%          surf(xr,yr,zr);
         pause(0.0000001);
%          hold off;
...
%    hold on;
     ...
     zf=[z11,z22];
          xsize=size(xf);
     xstep = floor(xsize(2)/5);

     for ste=1:xstep:xsize(2)
%          q=0 : pi/50 : 2*pi;
%          w = 0 : pi/50 : 2*pi;
%          [q,w]=meshgrid(q,w);
%          xr= xf(ste)+rr*sin(q).*cos(w);
%          yr= yf(ste)+rr*sin(q).*sin(w);
%          zr = zf(ste)+rr*cos (q);
         DrawQuad( xf(ste), yf(ste), zf(ste), rr , 'r' , 'b')
%          surf(xr,yr,zr);
         pause(0.05);
     end
      sure=[xf ; yf ; zf];
